...
% projection. The default is Plate Carree.
...
%structure has vectors of lat and lon. Let's loop over them and plot.
...
%graphical functions you have learned to use).
...
%Here, issuing this command will give you a very nice and accurate picture
%of Oregon.

%Let's check on the area of Oregon. The polyarea function is a-MAY-zing.
OR_area=polyarea(ORx(1:end-1),ORy(1:end-1)); %note notation to avoid 1st and last points being the same
...
%vectors, and other things to it. Let's load some sample data:
clear all
close all
clc
