...
%% Scalebars
% A map is only good if it is well annotated. Presenting data in lat/lon
% coordinates is often a very nice touch, but it can also be nice to show a
% scale bar with something like miles or kilometers indicated. In Matlab
% you do this with scaleruler. Let's look at an example.
clear all; close all;

usamap("Florida")
geoshow("usastatelo.shp","FaceColor","yellow")

% now, let's add a scale in km. 
gs1 = scaleruler;
setm(gs1,"XLoc",-3.21e5,"YLoc",2.81e6,"MajorTick",0:200:600)

%several things there. You MUST specify Xloc and Yloc in projected
%coordinates. The origin of coordinate projections depends on the
%projection being used (UTM or other). So, this can be tricky. If you
%want/need help, you can 'extract' the lower left location of a desired
%scalebar position with something like:

% [x,y]=ginput(1);
% Then, redo the setm command with x and y following the XLoc and YLoc
% options.

%next, note that you can (let's illustrate with a second scale bar) totally
%control major and minor tick locations. AND, tick lengths, etc. In the
%example below, I am making the tick lengths a bit too long, just to
%illustrate the point.

gs2 = scaleruler("Units","nm");
setm(gs2,"XLoc",-3.2e5,"YLoc",2.78e6,"TickDir","down", ...
    "MajorTick",0:100:300,"MinorTick",0:25:50, ...
    "MajorTickLength",km2nm(50),"MinorTickLength",km2nm(25))

%% Inset maps
% When you are creating a 'site map' for a paper you write, assuming you
% did some field work somewhere (collected some data), or you did a
% modeling study of a certain area, it can be very useful to create a map
% that has (1) the zoomed in area you worked in, but also (2) a
% regional-to-global map that shows the larger context. Remember, not
% everyone will be familar with your local area. Here is an illustration of
% how you can do this:
clear all; close all;

states = readgeotable("usastatehi.shp");
%Extract the table row for Massachusetts. Find the latitude and longitude limits of the Massachusetts polygon shape.

ma = geocode("Massachusetts",states);
[latlim,lonlim] = bounds(ma.Shape);

%Create a map using the latitude and longitude coordinates. Set the face 
%frame color of the map to light blue. Display the USA boundaries as 
% light-brown polygons and the Massachusetts boundaries as green polygons.

figure
h1 = usamap(latlim,lonlim);
setm(h1,"FFaceColor","#B7E9F7") %these are hexadecimal color codes, by the way!
...
%add box showing extent on inset
latmin=latlim(1); latmax=latlim(2); lonmin=lonlim(1); lonmax=lonlim(2);
lat_corners = [latmin, latmin, latmax, latmax, latmin]; % Connects the points to form a closed shape
lon_corners = [lonmin, lonmax, lonmax, lonmin, lonmin];

% Plot the bounding box
plotm(lat_corners, lon_corners, 'r-', 'LineWidth', 2)
